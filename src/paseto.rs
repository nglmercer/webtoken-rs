...
pub fn internal_verify_local(token: String, secret: String) -> Result<Map<String, Value>, String> {
    let key = derive_symmetric_key(&secret);
...
pub fn internal_create_public(payload: Map<String, Value>, secret_key_hex: String, expires_in_seconds: Option<i64>) -> Result<String, String> {
    let mut claims = Claims::new().map_err(|e| format!("PASETO claims error: {}", e))?;
    populate_claims(&mut claims, payload, expires_in_seconds)?;
...
pub fn internal_verify_public(token: String, public_key_hex: String) -> Result<Map<String, Value>, String> {
