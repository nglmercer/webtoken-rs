...
fn get_identifiers<'a>(client_id: &'a Option<String>, server_id: &'a Option<String>) -> Identifiers<'a> {
    Identifiers {
...
pub fn internal_server_register_start(
    ...
    let res = ServerRegistration::<DefaultCipherSuite>::start(&server_setup, request, client_id.as_bytes())
...
pub fn internal_client_register_finish(
    ...
    let res = state.finish(&mut rng, password.as_bytes(), response, params)
...
pub fn internal_server_login_finish(
    ...
    let res = state.finish(finalization, ServerLoginParameters::default())
