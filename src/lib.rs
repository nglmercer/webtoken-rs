...
#[cfg(feature = "napi-base")]
#[napi]
pub async fn scrypt_hash(
    password: String,
    log_n: Option<u8>,
    r: Option<u32>,
    p: Option<u32>,
) -> Result<String> {
    tokio::task::spawn_blocking(move || scrypt::internal_scrypt_hash(password, log_n, r, p))
        .await
        .map_err(|e| Error::from_reason(format!("Task join error: {}", e)))?
        .map_err(|e| Error::from_reason(e))
}
...
pub async fn compare(password: String, hash: String) -> Result<bool> {
    tokio::task::spawn_blocking(move || argon::internal_compare(password, hash))
        .await
        .map_err(|e| Error::from_reason(format!("Task join error: {}", e)))?
        .map_err(|e| Error::from_reason(e))
}
...
pub async fn scrypt_compare(password: String, hash: String) -> Result<bool> {
    tokio::task::spawn_blocking(move || scrypt::internal_scrypt_compare(password, hash))
        .await
        .map_err(|e| Error::from_reason(format!("Task join error: {}", e)))?
        .map_err(|e| Error::from_reason(e))
}
...
pub fn create(
    ...
    paseto::internal_create_local(payload, secret, expires_in_seconds)
        .map_err(|e| Error::from_reason(e))
}
...
pub fn create_public(
    ...
    paseto::internal_create_public(payload, &secret_key_hex, expires_in_seconds)
        .map_err(|e| Error::from_reason(e))
}
...
pub fn opaque_client_register_start(password: String) -> Result<OpaqueRegisterStartResult> {
    let res =
        opaque::internal_client_register_start(&password).map_err(|e| Error::from_reason(e))?;
...
pub fn opaque_server_register_start(
    ...
    opaque::internal_server_register_start(&server_setup_hex, &request_hex, &client_id)
        .map_err(|e| Error::from_reason(e))
}
...
pub fn opaque_client_register_finish(
    ...
    let res = opaque::internal_client_register_finish(
        &password,
        &response_hex,
        &state_hex,
        client_id,
...
pub fn opaque_server_login_start(
    ...
    let res = opaque::internal_server_login_start(
        &server_setup_hex,
        &password_file_hex,
        &request_hex,
        &client_id,
...
pub fn opaque_client_login_finish(
    ...
    let res = opaque::internal_client_login_finish(
        &password,
        &response_hex,
        &state_hex,
        client_id,
...
pub fn opaque_server_login_finish(finalization_hex: String, state_hex: String) -> Result<String> {
    opaque::internal_server_login_finish(&finalization_hex, &state_hex)
        .map_err(|e| Error::from_reason(e))
}
...
pub fn decode_token(_token: String) -> Result<Map<String, Value>> {
    Err(Error::from_reason(
        "PASETO local tokens are encrypted and cannot be decoded without the secret key.",
    ))
}
